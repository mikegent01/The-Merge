                ...
                frame: # Frame around the grid section
                    ...
                    vbox:
                        ...
                        python:
                            ...
                            _grid_cols = 4
                            _grid_rows = (_max_slots_display // _grid_cols) + (1 if _max_slots_display % _grid_cols else 0)
                        text f"Weight: {_inv_weight:.1f} kg" size 16 xalign 0.5 color "#CCCCCC"
                        text f"Slots: {len(inventory)} / {_max_slots_display}" size 16 xalign 0.5 color "#CCCCCC"
                        ...
                        viewport:
                            ...
                            grid _grid_cols _grid_rows:
                                ...
                                for i in range(_max_slots_display):
                                    if i < len(inventory):
                                        $ item_id = inventory[i]
                                        python:
...
screen item_quick_menu(item_id, item_details, img_path):
    modal True
    zorder 10 # Ensure it overlays
    ...
    frame:
        ...
        vbox:
            spacing 10
            text f"{item_id}" size 24 xalign 0.5 color "#FFFFFF" bold True
            ...
            frame:
                ...
                if renpy.loadable(img_path):
                    add img_path xalign 0.5 yalign 0.5 zoom 0.8
                else:
                    text item_id size 16 xalign 0.5 yalign 0.5

            text f"Type: {item_details.get('type', 'N/A')}" size 16 color "#CCCCCC" xalign 0.5
            text f"Weight: {get_item_weight(item_id)} kg" size 16 color "#CCCCCC" xalign 0.5
            text f"Desc: {get_item_description(item_id)}" size 16 color "#DDDDDD" xalign 0.5 text_align 0.5

            null height 10

            textbutton "Equip" action [Hide("item_quick_menu"), Show("slot_selection_screen", item_id=item_id)] style "inventory_button"
            textbutton "Use" action [Function(use_item, item_id), Hide("item_quick_menu"), Hide("inventory")] style "inventory_button"
            textbutton "Discard" action [Function(remove_item, item_id), Hide("item_quick_menu")] style "inventory_button"
            textbutton "Close" action Hide("item_quick_menu") style "inventory_button"
    ...
    if details:
        vbox:
            ...
            hbox:
                ...
                textbutton "Discard" action [Function(remove_item, item_id), SetVariable("selected_item", None)] style "inventory_button"
...
screen equipped_items_panel():
    vbox:
        spacing 20 # Increased for better layout
        # Head
        hbox:
            xalign 0.5
...
screen equipment_slot_display(slot_name, current_item_id):
    ...
    frame:
        style "slot_frame"
        xysize (120, 120) # Slightly larger for visibility
        ...
        vbox:
            if details:
                ...
                hbox:
                    xalign 0.5
                    spacing 10
                    textbutton "U" action [Function(use_item, current_item_id), Hide("inventory")] style "inventory_button" text_size 12 padding (4,4) tooltip "Use"
                    textbutton "X" action Function(unequip_item, slot_name) style "inventory_button" text_size 12 padding (4,4) tooltip "Unequip"
            else:
            ...
            if current_mode == "construction":
                                    ...
                                    if item_id not in selected_items:
                                        ...
                                        if details:
                                            hbox:
                                                ...
                                                textbutton "Select" action Function(selected_items.append, item_id) style "inventory_button" text_size 14 padding (5,2) sensitive (len(selected_items) < 2)
                    ...
                    vbox:
                        ...
                        frame:
                            ...
                            if not selected_items:
                                text "Select up to 2 items" xalign 0.5 yalign 0.5 color "#AAAAAA"
                            else:
                                hbox:
                                    spacing 20
                                    xalign 0.5
                                    for sel_id in selected_items:
                                        ...
                                        if sel_details:
                                            vbox:
                                                ...
                                                textbutton "Remove" action Function(selected_items.remove, sel_id) style "inventory_button" text_size 12 padding (5,2)
                ...
                vbox:
                    ...
                    viewport:
                        ysize 400 xsize 700
                        scrollbars "vertical" mousewheel True
                        grid 5 max(1, (len(inventory) + 4) // 5):  # Dynamic rows
                            spacing 10
                            xfill True
                            for item_id in inventory:
                                python:
                                ...
                                if _can_deconstruct and _details:
                                    button:
                                        style "slot_frame"
                                        action [Function(deconstruct_item, item_id), Hide("crafting_screen")]
                                        tooltip _tooltip_text
                                        if renpy.loadable(_img_path):
                                    ...
                                    frame:
                                        ...
                                        background "#444444"
                                        tooltip _tooltip_text
                                        if _details and renpy.loadable(_img_path):
                            ...
                            $ remaining = (5 * max(1, (len(inventory) + 4) // 5)) - len(inventory)
                            for i in range(remaining):
                                null

            null height 20
            textbutton "Close Workshop" action [SetVariable("selected_items", []), SetVariable("selected_holding_item", None), Hide("crafting_screen")] style "inventory_button"
